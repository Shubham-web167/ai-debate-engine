(function() {
  class GeminiAdapter extends self.BaseAdapter {
    constructor() {
      super('gemini');
    }

    async isLoggedIn() {
      // Gemini usually has signin URLs or buttons
  ...
  const selectorsPromise = fetch(url)
     .then(r => r.json())
     .then(selectors => {
       adapter.setSelectors(selectors.gemini);
       return selectors;
     })
     .catch(err => {
       console.error("Error setting up Gemini selectors:", err);
       throw err;
     });
